...
;General
  ...
  !define ConfabVer 0.2b

  ;Name and file
  Name "Confab ${ConfabVer}"
  OutFile "Confab${ConfabVer}_Installer.exe"

  ;Default installation folder
  InstallDir "$PROGRAMFILES\Confab-${ConfabVer}"
  
  ;Get installation folder from registry if available
  InstallDirRegKey HKCU "Software\Confab ${ConfabVer}" ""
...
;Pages
  ...
  !define MUI_STARTMENUPAGE_REGISTRY_KEY "Software\Confab ${ConfabVer}" 
...
Section "Dummy Section" SecDummy
  ...
  File /r ..\build\Release\confab.exe
  File /r ..\build\Release\calcrmsd.exe
  File /r ..\build\Release\formats_common.obf
  File /r ..\build\Release\plugin_forcefields.obf
  ...
  File /r /x .svn ExampleFiles\serotonin.mol
  
  ;Store installation folder
  WriteRegStr HKCU "Software\Confab ${ConfabVer}" "" $INSTDIR
  ...
  !insertmacro MUI_STARTMENU_WRITE_BEGIN Application
    ...
    CreateShortCut "$SMPROGRAMS\$STARTMENU_FOLDER\Confab Folder.lnk" "$INSTDIR"
...
Section "Uninstall"
  ...
  Delete "$INSTDIR\confab.exe"
  Delete "$INSTDIR\calcrmsd.exe"
  Delete "$INSTDIR\*.obf"
  ...
  DeleteRegKey /ifempty HKCU "Software\Confab ${ConfabVer}"
